...
public class Step1_cmd1_SO {
	...
	public Step1_cmd1_SO() {
		...
		this.ori_CMDPath = myUtil.setPath(linkageFolder,cmdFileListObj.getFileName(CMDFileList.cmd1_SO));
		String AdamsCommand = myUtil.setPath(MC.getWorkspace(), DatabaseFolderName.AdamsCommand); 
		this.new_CMDPath = myUtil.setPath(AdamsCommand, cmdFileListObj.getFileName(CMDFileList.cmd1_SO));
	...
	private void readAndSwapCMD_SerialOpposed(){
		...
		for(String line : reader.getFileDataList()){
			if(line.contains(CmdSwapParameter.SelectedBladeDBPath)){
				String newLine = line.replace(CmdSwapParameter.SelectedBladeDBPath, MC.getDBStep1().getSelectedBladeDBPath());
				this.outputDataList.add(newLine);
			}else if(line.contains(CmdSwapParameter.SelectedLinkageDBPath)){
				String newLine = line.replace(CmdSwapParameter.SelectedLinkageDBPath, MC.getDBStep1().getSelectedLinkageDBPath());
				this.outputDataList.add(newLine);
			}else{
