...
public class Step4_cmd4_BladePosition_SO {
	...
	public Step4_cmd4_BladePosition_SO() {
		...
		this.ori_CMDPath = myUtil.setPath(linkageFolder,cmdFileListObj.getFileName(CMDFileList.cmd4_BladePosition_SO));
		String AdamsCommand = myUtil.setPath(MC.getWorkspace(), DatabaseFolderName.AdamsCommand); 
		this.new_CMDPath = myUtil.setPath(AdamsCommand, cmdFileListObj.getFileName(CMDFileList.cmd4_BladePosition_SO));
	}
	...
	private void readAndSwapCMD_SerialOpposed(){
		...
		for(String line : reader.getFileDataList()){
			...
			}else if(line.contains(CmdSwapParameter.BladePositionPath)){
				String resultFolder = myUtil.setPath(MC.getWorkspace(), DatabaseFolderName.Result);
				String nodeInformationFolder = myUtil.setPath(resultFolder, DatabaseFolderName.BladePosition);
				//String partName = obj.getPartName();
				//String partNameFolder = myUtil.setPath(nodeInformationFolder, partName);
				String convert_jobSpaceFolder = nodeInformationFolder.replace("\\", "/");

				String newLine = line.replace(CmdSwapParameter.BladePositionPath, convert_jobSpaceFolder);
				this.outputDataList.add(newLine);
			}else{
