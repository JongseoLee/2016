...
def create_nodes_2d(x,y,z,sn) :
   py_send("*add_nodes %f , %f , %f " %(x, y ,0.0))  # n0 : Center Node of Roll
   py_send("*add_nodes %f , %f , %f " %(x+50.0, y ,0.0)) # n1 : Aux. Node of Roll 
   cname=sn.split("_")
   if (cname[0]== "UpperRoll" ) : 
     py_send("*add_nodes %f , %f , %f " %(x-50.0, y-50.0 ,0.0)) # n2 : 1st node of spring element
     py_send("*add_nodes %f , %f , %f " %(x+50.0, y-50.0 ,0.0)) # n3 : 2nd node of spring element
     py_send("*add_nodes %f , %f , %f " %(x+50.0, y+50.0 ,0.0)) # n4 : 3rd node of spring element      
     py_send("*add_nodes %f , %f , %f " %(x-50.0, y+50.0 ,0.0)) # n5 : 4th node of spring element
     py_send("*add_nodes %f , %f , %f " %(x, y+50.0 ,0.0))  # n6 : Fixed Point of Roll
...
def create_spring_2d(n0,e0,sn,nodes_up,nodes_low,mvar) :
  cname=sn.split("_")
  py_send("*add_elements %i %i %i %i  " %(n0+2,n0+3,n0+4,n0+5))
  ...
  py_send("*add_rbe2_tied_nodes %i %i #" %(n0+2,n0+3))
  py_send("*new_rbe2 *rbe2_name rbe2_"+sn+"_fixed ")
  py_send("*rbe2_ret_node %i " %(n0+6))
  py_send("*rbe2_tied_dof 1 *rbe2_tied_dof 2 *rbe2_tied_dof 3 *rbe2_tied_dof 4 *rbe2_tied_dof 5 *rbe2_tied_dof 6 ")
  py_send("*add_rbe2_tied_nodes %i %i  #" %(n0+4,n0+5))
  if (cname[0]== "UpperRoll" ) :
      nodes_up.append(n0)
      ...
      py_send("*add_apply_nodes %d # "%(n0+6))
  if (cname[0]== "LowerRoll" ) :      
      nodes_low.append(n0)
...
#
    ...
    py_send("*apply_dof x *apply_dof_value x %f" %uv) # 2D
    py_send("*apply_dof_table x rotation_table ")
    py_send("*new_apply *apply_type fixed_displacement ")
    py_send("*apply_name Lower_roll_rot ")
    py_send("*apply_dof x *apply_dof_value x %f" %lv) # 2D
    py_send("*apply_dof_table x rotation_table ")
...
def add_thick_info() :
	...
	py_send("*new_geometry *geometry_type mech_planar_pstrain @popdown(geom_new_popmenu) @popdown(geom_new_structural_pm)")
	py_send("*geometry_name Spring_elem ")
	py_send("*geometry_param norm_to_plane_thick 100 ")
	py_send("*select_clear *select_contact_body plate")
	py_send("*add_geometry_elements all_unselected ")
	py_send("*select_clear ")
...
def main():
  ...
  create_dummy_mats_2d()
  create_bcs_2d(upper_v,lower_v) # Spring Support Roll Rotation & Roll BCs   
  create_rmotion_bc_table(mvar)   
  py_send("*set_element_class quad4 ") # in 2D 
  ...
  #  print " Width of Plate ", py_get_float("W")
    ...
    create_nodes_2d(cx,cy,0.0,sn)
    n0,e0=nnode+(i-2)*7+1,nelem+(i-2)+1
    create_spring_2d(n0,e0,sn,nodes_up,nodes_low,mvar)
