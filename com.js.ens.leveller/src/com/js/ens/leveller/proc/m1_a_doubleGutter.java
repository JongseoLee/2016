...
public class m1_a_doubleGutter {
	private Logger log =Logger.getLogger(m1_a_doubleGutter.class);
	...
	public m1_a_doubleGutter(){
		plateType = LMain.getcomboType();
	}
	
	public m1_a_doubleGutter(String path){
	...
	public void running(){
		try{
			procFilePath = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "m1_a_doublegutter.py");
			copyProc();
			LMain.getExportMSG().addData("SUCCESS - Export(m1_a_doublegutter) \n path : "+ this.procFilePath);
			log.info("SUCCESS - Export(m1_a_doublegutter) \n path : "+ this.procFilePath);
		}catch(Exception e){
			String msg = "ERROR - Export(m1_a_doublegutter)";
			msg = msg +"\n"+e.getMessage();
	...
	public void copyProc() throws Exception{
		...
		String sourcePath = myUtil.setPath(s_folder, "m1_a_doublegutter.py");
