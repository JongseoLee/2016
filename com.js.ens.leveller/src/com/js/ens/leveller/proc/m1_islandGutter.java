...
public class m1_islandGutter {
	private Logger log = Logger.getLogger(m1_islandGutter.class);
	...
	public m1_islandGutter(){
		plateType = LMain.getcomboType();
	}
	
	public m1_islandGutter(String path){
	...
	public void running(){
		try{
			//
			procFilePath = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "m1_islandgutter.proc");
			copyProc();
			LMain.getExportMSG().addData("SUCCESS - Export(m1_islandgutter) \n path : "+ this.procFilePath);
			log.info("SUCCESS - Export(m1_islandgutter) \n path : "+ this.procFilePath);
			
			m1_a_islandGutter m1Obj = new m1_a_islandGutter();
			m1Obj.running();
			
			String modulePath = myUtil.setPath(System.getProperty("user.dir"), "module");
			String inpy1 = myUtil.setPath(modulePath,"mp01_define_post_set_islandgutter.py");
			String inpy2 = myUtil.setPath(modulePath,"mp02_redefine_post_set_islandgutter.py");
			String outpy1 = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "mp01_define_post_set_islandgutter.py");
			String outpy2 = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "mp02_redefine_post_set_islandgutter.py");
			myUtil.fileCopy(inpy1, outpy1);
			myUtil.fileCopy(inpy2, outpy2);
		} catch(Exception e){
			String msg = "ERROR - Export(m1_islandgutter)";
			msg = msg +"\n"+e.getMessage();
	...
	public void copyProc() throws Exception{
		...
		String sourcePath = myUtil.setPath(s_folder, "m1_islandgutter.proc");
