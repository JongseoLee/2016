...
public class a4_contact {
	private Logger log = Logger.getLogger(a4_contact.class);
	...
	public a4_contact(){
		...
		//flip_surface_List.add(AllRollNum+1);
		...
		/*
		for(int j=AllRollNum;j<=AllRollNum+4;j++){
			add_contact_body_surfaces_List.add(j);
		}
		for(int C_bodySurNum : add_contact_body_surfaces_List){
			if(flip_surface_List.contains(C_bodySurNum)){
				flip_surface_List.remove(C_bodySurNum);
			}
		}
		int lastNum = flip_surface_List.size();
		for(int k = 0; k<=flip_surface_List.size()-1;k++){
			if(k == lastNum-1){
				flip_surfaces_positiveNum += flip_surface_List.get(k);
			}else {
				flip_surfaces_positiveNum += flip_surface_List.get(k)+",";
			}
		}
	...
	public a4_contact(String path){
		AllRollNum = Integer.parseInt(LMain.getSpinnerUpperRollNum())+Integer.parseInt(LMain.getSpinnerLowerRollNum());
		add_contact_body_surfaces = (AllRollNum+1)+", "+(AllRollNum+2);
		for(int i=0;i<AllRollNum;i++){
			flip_surfaces += (i+1)+",";
		}
		flip_surfaces+=String.valueOf(AllRollNum+1);
	...
	public void running(){
		try{
			// 실제 사용 
			procFilePath = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "a4_contact.proc");
	...
	public void readSourceData() throws Exception{
		...
		String sourcePath = myUtil.setPath(s_folder, "a4_contact.proc");
	...
	public void swapValue() throws Exception{
		...
		for(String line : fileDataList){
			if(line.contains("%"+ProcVariable.upperRollGenContact+"%")){
				...
				continue;
			}else if(line.contains("%"+ProcVariable.lowerRollGenContact+"%")){
				...
				continue;
			}else if(line.contains("%"+ProcVariable.add_contact_body_surfaces+"%")){
				newLine = line.replace("%"+ProcVariable.add_contact_body_surfaces+"%", this.add_contact_body_surfaces);
				procDataList.add(newLine);
				continue;
			}else if(line.contains("%"+ProcVariable.flip_surface+"%")){
				//update version2
				if(Double.parseDouble(LMain.getTextRollCrown()) > 0.0){
					newLine = line.replace("%"+ProcVariable.flip_surface+"%", this.flip_surfaces_positiveNum);
					procDataList.add(newLine);
				}else{
					newLine = line.replace("%"+ProcVariable.flip_surface+"%", this.flip_surfaces);
					procDataList.add(newLine);
				}			
				continue;
			}else if(line.contains("%"+ProcVariable.plateRollGenContactTable+"%")){
				for(int i=0;i<Integer.parseInt(LMain.getSpinnerUpperRollNum());i++){
