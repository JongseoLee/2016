...
public class m1_partialGutter {
	private Logger log = Logger.getLogger(m1_partialGutter.class);
	...
	public m1_partialGutter(){
		plateType = LMain.getcomboType();
	}
	
	public m1_partialGutter(String path){
	...
	public void running(){
		try{
			//
			procFilePath = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "m1_partialgutter.proc");
			copyProc();
			LMain.getExportMSG().addData("SUCCESS - Export(m1_partialgutter) \n path : "+ this.procFilePath);
			log.info("SUCCESS - Export(m1_partialgutter) \n path : "+ this.procFilePath);
			
			m1_a_partialGutter m1Obj = new m1_a_partialGutter();
			m1Obj.running();
			
			String modulePath = myUtil.setPath(System.getProperty("user.dir"), "module");
			String inpy1 = myUtil.setPath(modulePath,"mp01_define_post_set_partialgutter.py");
			String inpy2 = myUtil.setPath(modulePath,"mp02_redefine_post_set_partialgutter.py");
			String outpy1 = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "mp01_define_post_set_partialgutter.py");
			String outpy2 = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "mp02_redefine_post_set_partialgutter.py");
			myUtil.fileCopy(inpy1, outpy1);
			myUtil.fileCopy(inpy2, outpy2);
		}catch(Exception e){
			String msg = "ERROR - Export(m1_partialgutter)";
			msg = msg +"\n"+e.getMessage();
	...
	public void copyProc() throws Exception{
		...
		String sourcePath = myUtil.setPath(s_folder, "m1_partialgutter.proc");
