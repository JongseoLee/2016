...
public class m1_centerWave {
	private Logger log = Logger.getLogger(m1_centerWave.class);
	...
	public m1_centerWave(){
		plateType = LMain.getcomboType();
	}
	
	public m1_centerWave(String path){
	...
	public void running(){
		try{
			//
			procFilePath = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "m1_centerwave.proc");
			copyProc();
			
			LMain.getExportMSG().addData("SUCCESS - Export(m1_centerwave) \n path : "+ this.procFilePath);
			log.info("SUCCESS - Export(m1_centerwave) \n path : "+ this.procFilePath);
			
			m1_a_centerWave m1Obj = new m1_a_centerWave();
			m1Obj.running();
			
			String modulePath = myUtil.setPath(System.getProperty("user.dir"), "module");
			String inpy1 = myUtil.setPath(modulePath,"mp01_define_post_set_centerwave.py");
			String inpy2 = myUtil.setPath(modulePath,"mp02_redefine_post_set_centerwave.py");
			String outpy1 = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "mp01_define_post_set_centerwave.py");
			String outpy2 = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "mp02_redefine_post_set_centerwave.py");
			myUtil.fileCopy(inpy1, outpy1);
			myUtil.fileCopy(inpy2, outpy2);
		}catch(Exception e){
			String msg = "ERROR - Export(m1_centerwave)";
			msg = msg +"\n"+e.getMessage();
	...
	public void copyProc() throws Exception{
		...
		String sourcePath = myUtil.setPath(s_folder, "m1_centerwave.proc");
