...
public class m1_a_singleGutter {
	private Logger log = Logger.getLogger(m1_a_singleGutter.class);
	...
	public m1_a_singleGutter(){
		plateType = LMain.getcomboType();
	}
	
	public m1_a_singleGutter(String path){
		this.procFilePath = path;
		plateType = LMain.getcomboType();
		/*
		 *  procdata를 생성하여 파일로 직접 writer 하지 않고 
		 *  원본 파일 복사하여 재사용
		 * 
		 */
	}
	...
	public void running(){
		...
		try{
			procFilePath = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "m1_a_singlegutter.proc");
			copyProc();
			LMain.getExportMSG().addData("SUCCESS - Export(m1_a_siglegutter) \n path : "+ this.procFilePath);
			log.info("SUCCESS - Export(m1_a_siglegutter) \n path : "+ this.procFilePath);
		}catch(Exception e){
			String msg = "ERROR - Export(m1_a_siglegutter)";
			msg = msg +"\n"+e.getMessage();
	...
	public void copyProc() throws Exception {
		...
		String sourcePath = myUtil.setPath(s_folder, "m1_a_singlegutter.proc");
