...
public class m1_a_flat {
	private Logger log = Logger.getLogger(m1_a_flat.class);
	...
	public void running(){
		try{
			procFilePath = myUtil.setPath(myUtil.setPath(LMain.getWorkspace(), "proc"), "m1_a_flat.proc");
			copyProc();
			
			LMain.getExportMSG().addData("SUCCESS - Export(m1_a_flat) \n path : "+ this.procFilePath);
			log.info("SUCCESS - Export(m1_a_flat) \n path : "+ this.procFilePath);
			
		}catch(Exception e){
			String msg = "ERROR - Export(m1_a_flat)";
			msg = msg +"\n"+e.getMessage();
	...
	public void copyProc() throws Exception{
		...
		String sourcePath = myUtil.setPath(s_folder, "m1_a_flat.proc");
