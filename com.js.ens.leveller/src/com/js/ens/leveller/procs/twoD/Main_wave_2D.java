...
import com.js.ens.leveller.procs.twoD.wave.a2_roll_gen_wave;
import com.js.ens.leveller.procs.twoD.wave.a3_material_define_wave;
import com.js.ens.leveller.procs.twoD.wave.a4_contact_wave;
import com.js.ens.leveller.procs.twoD.wave.a6_loadcase_wave;
import com.js.ens.leveller.procs.twoD.wave.define_parameters_wave;
import com.js.io.Reader;
import com.js.io.Writer;

public class Main_wave_2D {
	private Logger log = Logger.getLogger(Main_wave_2D.class);
	private LevellerMain LMain = LevellerMain.getInstatnce();
	
	private ArrayList<String> filePathList = new ArrayList<String>();
	private String main_wave_Path 						= "";
	...
	public void running(ArrayList <String> destFullPath){
		...
		try{
			...
			this.swapValue();
			this.writeProc();
			LMain.getExportMSG().addData("SUCCESS - Export(main_wave) \n path : "+ this.main_wave_Path);
			log.info("SUCCESS - Export(main_wave) \n path : "+ this.main_wave_Path);
			
			this.GenOtherProcs();
			
		}catch(Exception e){
			String msg = "ERROR - Export(main_wave)";
			msg = msg +"\n"+e.getMessage();
	...
	private void setFilesPath() throws Exception{
		for(String path : this.filePathList){
			if(path.contains("main_wave_2d.proc")){
				this.main_wave_Path = path.trim();
	...
	private void readSourceData() throws Exception{
		...
		Reader reader = new Reader(this.main_wave_Path);
		reader.running();
	...
	private void writeProc() throws Exception{
		Writer writer = new Writer(this.main_wave_Path,this.procDataList);
		writer.running();
	}
	...
	private void GenOtherProcs(){
		...
		define_parameters_wave DPObj = new define_parameters_wave();
		DPObj.running(this.define_parameters_PATH);
		//a2_roll_gen.proc
		a2_roll_gen_wave a2Obj = new a2_roll_gen_wave();
		a2Obj.running(this.a2_roll_gen_PATH);
		//a3_material_define.proc
		a3_material_define_wave a3Obj = new a3_material_define_wave();
		a3Obj.running(this.a3_material_define_PATH, matsPath);
		//a4_contact.proc
		a4_contact_wave a4Obj = new a4_contact_wave();
		a4Obj.running(this.a4_contact_PATH);
		//a6_loadcase.proc
		a6_loadcase_wave a6Obj = new a6_loadcase_wave();
