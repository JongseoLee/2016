...
public class mat02_flowStress {
	...
	public void running(String type,String standType){
		...
		if(this.type.equals("constant")){
			...
			new_filePath = myUtil.setPath(myUtil.setPath(myUtil.setPath(MC.getWorkspace(), "proc"),standType),this.FileName_mat02Con);
		}else {
			ori_filePath = myUtil.setPath(myUtil.setPath(modulePath, standType), this.FileName_mat02Tbl);
			new_filePath = myUtil.setPath(myUtil.setPath(myUtil.setPath(MC.getWorkspace(), "proc"),standType),this.FileName_mat02Tbl);
	...
	private void writeProc(String destFolder) {
		...
		Writer writer = new Writer(destFolder,this.procDataList);
		writer.running();
	}
